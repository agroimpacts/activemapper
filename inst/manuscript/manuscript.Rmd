...
Both the per-pixel and vectorized maps, when combined with the map reference sample, enable separate estimates of the total extent of arable croplands in Ghana. The cropland extent estimated from the vectorized map is 42,359 km$^2$ (with a margin of error of 4,395 km$^2$), or `r round(42359 / (42359 + 204940) * 100, 1)`% of the mapped area, the estimate based on the per pixel map is 43,233 km$^2$ (margin of error = 4,904 km$^2$), or `r round(43233 / (43233 + 202856) * 100, 1)`% of area. 
...
Another potential issue is the degree to which our assessment of the impact of label error on model performance (Figure \ref{fig:trainingimpacts}) was influenced by the validation labels we used, which were generated using the consensus method. This could have confounded the assessment, particularly when comparing models trained with the most accurate individual label and those trained with consensus labels. However, the visual assessment of probability images generated from each labelling strategy show that these two strategies produce nearly identical maps, and that the model trained with low quality labels generated a markedly less certain map (Figure S6). 

## Looking forward 
- Provide new insight 
- Given errors, should be considered generation 1 maps
- Sets up capability for repeated annual mapping of croplands to understand critical issues of how these systems are changing (sizes and shapes of croplands, where they are expanding) (Jayne papers, recent agricultural expansion paper)
- Key components will be useful in other applications and regions
- Provides means for improving crop type and crop yield estimates
- Improvements to help realize these capabilities: 
-     [This suggests that replacing the Random Forest more advanced deep learning model]. promise of CNNs (cite Waldner paper, others), for segmentation to realize this. This systems is well designed to adapt to it, and capabilities for assessing various aspects of label quality, e.g. edge accuracy and fragmentation accuracy, can help improve quality of labels designed to emphasize edges
  - Can still retain the active architecture, but instead use new sites to improve a pre-trained model
  - Deep learning may be more resistant to label errors (cites)
- Map along agroecozone boundaries, rather than random blocks  
- Another interesting question to answer: whether guided approach to site selection would result in more informative sample for CNNs also.
- Higher number of high quality reference labels can achieve this. 
