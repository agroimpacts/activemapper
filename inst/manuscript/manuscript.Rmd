...
We generated a catalog of `r nrow(tile_key) * 2` composite tiles (hereafter simply "tiles") for Ghana, consisting of a seasonal pair for each of the `r nrow(tile_key)` 0.05$^\circ$ tile grid cells covering Ghana. To assess the quality of the resulting composites, `r tilen` tile grid cells were randomly selected, and two separate observers graded each corresponding seasonal composite using a four category scoring system, which evaluated the degree of 1) residual cloud and 2) cloud shadow, 3) the number of visible scene boundary artifacts, and 4) the proportion of the image that had its resolution degraded below the typical 3-4 m PlanetScope resolution (e.g. because of between-date image mis-registrations). Each category was qualitatively ranked from 0-3, with 0 being the lowest quality, and 3 the highest (see SI for complete protocol), making the highest possible score 12. We rescaled scores to fall between 0 and 1.
...
To make the initial maps of cropland probabilities, the active learning process ran for 3 iterations in 12 of 16 AOIs, varying from as little as 1 to as many as 4 iterations across the other 4 AOIs, with the number of iterations associated with the performance of the starter models against the validation labels (i.e. AOIs with higher starting performance stopped after fewer iterations, see SI). A total of `r ttrain` training and `r tval` validation labels were collected by 20 labellers to develop and assess model performance. Each AOI's model was trained by the 500 randomly selected labels, plus an additional `r aoi10n` - `r aoi15n` (typically `r typicaln`) labels within each the AOI that were selected by active learning. Actively selected labels showed distinctive patterns in several AOIs, such as concentrating along ecotones, including the boundaries between agro-ecozones (see Figure S4A).
