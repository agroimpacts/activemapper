...
After fitting, the model is applied to the features of the model validation set, and a set of accuracy metrics are calculated, including ordinary binary accuracy, the F1 score, and the area under the curve of the Receiver Operating Characteristic [@Pontiustotaloperatingcharacteristic2014]. 
...
The accuracy assessment procedure described in section 2.2 was used strictly to assess performance gains in the active learning loop, according to suggested  best practices [@ElmesAccountingtrainingdata2020]. To independently assess the categorical accuracy of final map products, we followed guidelines provided by [@StehmanKeyissuesrigorous2019] to create a map reference sample. To develop this sample, we first made a stratified random sample of grid cells across the entire country, using the mapped field boundaries to define the two strata. Next, a primary supervisor examined the image composites in each randomly selected cell, and placed a square polygon of ~0.1 ha on the primary class intersecting the centerpoint of the cell. The square was shifted so that it was contained entirety with the intersected class: cropland or non-cropland. Where the supervisor was uncertain about the class label, one of two additional classes were selected: likely cropland or likely non-cropland. After placing these polygons, the supervisor removed their classes from the polygons and sent them to three separate experts, who labelled the polygons according to their interpretation of the underlying imagery. The four sets of labels were then combined and the overall agreement between them was calculated. This agreement provided the upper bound on knowable map accuracy. See the SI for further details on the creation and analysis of the map reference sample. 

To assess the accuracy of the segmented field boundaries, we compared the size class distributions of the segmented field boundaries against those of the  workers' digitized polygons at map validation sites. We calculated the differences in the measures of central tendency (mean, median, mode) between the two datasets within each validation site, and at the scale of the AOI.  

To further evaluate the capacities of different components of the system, we performed several further experiments. In the first experiment, we evaluated the degree to which training label quality impacts final map accuracy, by comparing maps created by training the RandomForest algorithm with labels created by the least accurate worker, the most accurate worker, as well as those trained with the consensus labels. This test aligns with Tier 1 standards for accounting for the impact of training data error on final map accuracy [@ElmesAccountingtrainingdata2020]. Next, we evaluated the degree to which training sample selection via active learning impacted map accuracy relative to random selection of additional training sites. Lastly, we repeated the entire map development and map assessment process for one AOI for the 2019 agricultural year, in order to assess the ability of the system to create annually updated maps and detect inter-annual change. 
...
## The characteristics of Ghanaian cropland
Using the final mapped results, we calculated the estimated area of arable cropland in Ghana, as well as the characteristics of field sizes through the country. We further used the uncertainty information encoded in the cropland probability images produced by the Random Forest classifier to provide further insight into the characteristics of cropland active-fallow cycles.  
...
After each site was mapped by four labellers, consensus labels were generated. **The average risk for each consensus label was *X* (see SI)**.  
...
# AOI score ranges
imet_min <- iteration_metrics %>% group_by(AOI) %>%
...
imet_max <- iteration_metrics %>% group_by(AOI) %>%
  filter(Iteration == max(Iteration) & !grepl("change", Metric)) %>%
  filter(Metric %in% c("Accuracy", "AUC", "F1")) %>% 
  ungroup() %>% 
  group_by(Metric) %>% 
...
The gain in model performance was assessed for each AOI against several accuracy metrics. The average accuracy, AUC, and F1 at Iteration 0 were `r imet_mu0 %>% filter(Metric == "Accuracy") %>% pull(Score)`, `r imet_mu0 %>% filter(Metric == "AUC") %>% pull(Score)`, and `r imet_mu0 %>% filter(Metric == "F1") %>% pull(Score)`, respectively, increasing to `r imet_mu3 %>% filter(Metric == "Accuracy") %>% pull(Score)`, `r imet_mu3 %>% filter(Metric == "AUC") %>% pull(Score)`, and `r imet_mu3 %>% filter(Metric == "F1") %>% pull(Score)` by Iteration 3 (Figure \ref{fig:alperformance}). These differences represent respective gains of `r imet_mu3 %>% filter(Metric == "Accuracy") %>% pull(dscore)`, `r imet_mu3 %>% filter(Metric == "AUC") %>% pull(dscore)`, and
`r imet_mu3 %>% filter(Metric == "F1") %>% pull(dscore)` for the three metrics. The largest gains in each metric were registered on Iteration 1, averaging `r imet_dmu1 %>% filter(Metric == "Accuracy") %>% pull(Score)`, `r imet_dmu1 %>% filter(Metric == "AUC") %>% pull(Score)`, and `r imet_dmu1 %>% filter(Metric == "F1") %>% pull(Score)` percent for accuracy, AUC, and F1. The lowest gains were received on Iteration 3 for accuracy (`r imet_dmu3 %>% filter(Metric == "Accuracy") %>% pull(Score)`%) and F1 (`r imet_dmu3 %>% filter(Metric == "F1") %>% pull(Score)`%), while Iteration 2 showed the lowest average gain for AUC (`r imet_dmu3 %>% filter(Metric == "AUC") %>% pull(Score)`%). The range in scores varied substantially across AOIs and metrics. Accuracy ranged between `r imet_min %>% filter(Metric == "Accuracy") %>% pull(Score)` (AOI `r imet_min %>% filter(Metric == "Accuracy") %>% pull(AOI)`) and `r imet_max %>% filter(Metric == "Accuracy") %>% pull(Score)` (AOI `r imet_max %>% filter(Metric == "Accuracy") %>% pull(AOI)`), while AUC varied from `r imet_min %>% filter(Metric == "AUC") %>% pull(Score)` (AOI `r imet_min %>% filter(Metric == "AUC") %>% pull(AOI)`) and `r imet_max %>% filter(Metric == "AUC") %>% pull(Score)` (AOI `r imet_max %>% filter(Metric == "AUC") %>% pull(AOI)`), and F1 from `r imet_min %>% filter(Metric == "F1") %>% pull(Score)` (AOI `r imet_min %>% filter(Metric == "F1") %>% pull(AOI)`) and `r imet_max %>% filter(Metric == "F1") %>% pull(Score)` (AOI `r imet_max %>% filter(Metric == "F1") %>% pull(AOI)`).
...
To evaluate the accuracy of the segmented field boundaries, we calculate the mean and standard deviation of the most accurate (for the given AOI) labeller's maps for validation sites, calculated the mean and standard deviation of area, and compared these statistics to those derived from the model-generated segments for those sites (Figure X). 
