...
The vectorized map provides additional information on how the characteristics of croplands can vary geographically, ranging from narrow, strip-like fields in parts of AOI 15 (Figure 7's lower right inset) to more densely packed, less distinctly shaped fields in area such as AOI 5 (upper right inset in Figure 7). These characteristics can be summarized and mapped to explore how such characteristics vary geographically For example, average fields sizes were mapped at 0.005 degree resolution (Figure S12A), providing some indication of countrywide patterns. Given the over-estimation of fields sizes relative to the validation labels, particularly in several savanna-zone AOIs (2, 3, 6, 7, and 15), the mean sizes likely reflect agglomerations of contiguous smallholder fields, such as in AOI 2,and the southern portion of AOI 8 (Figure S12A). A closer approximation of individual field boundaries can be mapped using approaches such as regression kriging to model the relationship between the segmented boundaries and the hand drawn labels collected at validation points (Figure S12B), although such an approach introduces spatial artifacts. 
