...
```{r acctabs, echo = FALSE, out.width="90%", fig.cap="Accuracy results", fig.align='center', message=FALSE}
...
## Ghana's croplands
Two separate maps of cropland were produced for each AOI, one derived from the cropland probability map, and the other from the segmented field boundaries. 

Map figure:
- Panel A: Cropland probability map (0.005 degree resolution)
  - Panel A insets: Probability close-ups in AOIs 1, 8, 13, 15
- Panel B: Cropland percentage map (0.005 degree)
  - Panel B insets: Segment close-ups in AOIs 1, 8, 13, 15
- Panel C: Median field size
- Panel D: Field number
- Median field size
- Cropland percentage

Stats figure: 
- Mean field size per AOI
- Median field size per AOI
- SD fields per AOI 
- N field per AOI
