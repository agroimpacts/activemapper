...
- Map at large scale using PlanetScope, one of the first (any citations). 
  - Done using small sat data for a single agricultural year, for two primary seasons
  - Follows recommended best practice for training ML algorithms

- Takes step towards answering important questions about the characteristics of fields and how many there are. 
  - Provide insights into the characteristics of croplands
  - Discussion about the characteristics of croplands: estimates of field sizes
  - What is says about Ghana's croplands. 

## Performance of the system
- Pixel-wise system, reasonable overall accuracy, but low performance on the agricultural class
- Low performance primarily due to several factors:
  - Model simplicity and scale of compute
  - Challenge of labelling task - uncertainty in labels
    - Should also be noted substantial uncertainty in identifying fields (map reference uncertainty)
  - Difficulty of getting clean imagery in highly cloudy areas
- Design of system helped to mitigate these factors:
  ...
  - Image quality:
    - Developed our own mean-weighting not median composite (artifacts) (maybe methods)
    - impact on accuracy--residual cloud issues
    - Important to develop the method because best-on-top method of Planet's basemaps leaves lots of residual cloud in key seasons, and require large volumes of imagery in highly cloudy areas such as southerwestern Ghana.
  
- Segmentation accuracy issues:
  - Over-merging
  - Bias in mean sizes likely reflect agglomerations of contiguous smallholder fields, and the difficulty of segmenting indistinct adjacent boundaries. Such as in AOI 2,and the southern portion of AOI 8 (Figure S12A).
  - Importance of having high quality probabilities underlying for filtering segments
...
- Improvements to help realize these capabilities: promise of CNNs (cite Waldner paper, others), for segmentation to realize this. This systems is well designed to adapt to it, and capabilities for assessing various aspects of label quality, e.g. edge accuracy and fragmentation accuracy, can help improve quality of labels designed to emphasize edges
