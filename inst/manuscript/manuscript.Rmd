...
\end{equation}
```
Which is a weighted mean for each pixel for each band *B* for the particular compositing period. The composited tiles were then added to the S3 store (Figure \ref{fig:systemoverview}), where they are stored as cloud-optimized geotiffs, and a "slippy map\footnote{https://wiki.openstreetmap.org/wiki/Slippy\_Map}" rendering is created for each composite using Raster Foundry [@azaveaRasterFoundry2020]. The web-rendered imagery is presented within the training data platform (next section).
