...
cap <- glue::glue("The average size and total number of crop fields for each ", 
                  "AOI and for Ghana overall. The original and bias-adjusted values", 
                  " for each measure are provided, as well as the total number", 
                  " of 0.05$^\\circ$ degree tiles in each AOI.")
...
Another major limiting factor was the inherent difficulty of the labelling task. The system is designed to minimize the error inherent in labelling, but determining croplands from non-croplands over these system is difficult and requires educated guessing by each labeller. Interpretation is particularly hard over areas where the background savanna vegetation and croplands have similar reflectance during the dry season, which is a particular problem in AOIs 2 and 3. Such changes in scene circumstances require labellers to adopt different heuristics, for example by delineating the field boundary on the growing season rather than the dry season image. Smaller field sizes also complicated the labelling task, as smaller fields become increasingly indistinct in the ~4 m PlanetScope composites. These challenges caused substantial differences in labels, as evident in the Bayesian Risk metrics (Figure S5) and labeller accuracies that averaged 71%, as measured against our reference set. The design of our system allowed us to measure how much label quality impacts model performance. Although prior work [@mellorExploringIssuesTraining2015; @rodriguez-galianoAssessmentEffectivenessRandom2012] has reported that RandomForest is particularly robust to training data error, we found its impact to be substantial (Figure \href{fig:trainingimpact}), suggesting that the greatest single investment towards improved accuracy lies in improving training data quality. 

Image quality was another issue, although primarily in the forest zone (Zone 4) AOIs, where frequent cloud cover reduced the number of available images in all seasons, resulting in composites with more brightness artifacts and blur (Figure \href{fig:imqual}). This impacted labellers' abilities to discern arable fields, and doubtless affected model predictions. There is little to be done to mitigate these errors, short of confining imagery to the less cloudy dry season, which may further undermine model performance--prior work shows that imagery from multiple seasons is important for improving classifier performance [@Debatsgeneralizedcomputervision2016; @Defournyrealtimeagriculturemonitoring2019]. Alternatively, composite quality could be improved by integrating imagery from the same seasons over multiple years, but this would undermine the goal of developing annual maps, while the dynamism of the croplands would confound the ability to delineate field boundaries. Alternative compositing approaches might also be ineffective, as the "best-on-top" approach\footnote{https://assets.planet.com/products/basemap/planet-basemaps-product-specifications.pdf} used to develop PlanetScope basemaps tends to result in substantial residual clouds in this region.  

The final major error sources relates to the segmentation algorithm, which had two particular problems. The first of these relates to the substantial commission errors in the vector maps, which is attributable to errors in the RandomForest predictions. Uncertainty in that model resulted in substantial numbers of pixels just above the 0.5 classification threshold, meaning a number of segments over highly uncertain areas were retained as fields. This problem would resolve with a more accurate classifier, or potentially by using locally varying thresholds for distinguishing croplands from non-cropland [e.g. @waldnerDeepLearningEdge2020].  The second problem was over-merging by the segmentation algorithm, which caused over-estimation of field sizes and often irregular shapes, particularly in high density croplands, such as in AOIs 2 and 8 (Figure \href{fig:mainmap}), where closely adjacent small-scale fields have boundaries that are often indistinct in the PlanetScope imagery. In this case, minimizing or preventing merging could help, resulting in an over-segmented solution that would likely underestimate field sizes. However, from the standpoint of other map applications, such as filtering pixels for crop type mapping or yield estimation, over-segmented solutions could help by isolating distinct within field units, such as those caused by soil type differences. 

## Key system features
- Design of system helped to mitigate these factors:
  - Consensus labels minimize error substantially
  - Active learning selects cites more efficiently, some evidence it improves more efficiently and overall relative to random selection. Less marked gain here, but doing this at much larger scale than Debats
  - Multi-season imagery helps improve it    
    - Feng et al (2020) importance of temporal information (backs up Debats et al).
    - temporal information makes labelling harder though
  - Image quality:



Although the raw maps are biased, data from the labelling platform provides information that can be used to correct for these discrepancies.
The detail, large extent, and temporal precision of the map were enabled by the system's ability to process small-sat data, which currently provide the only wall-to-wall, sub-5 meter imaging capability [@McCabefutureEarthobservation2017] with the frequency necessary to create clean imagery in areas of frequent cloud cover across multiple crop growing periods, which is important for distinguishing cropland from non-cropland [@Debatsgeneralizedcomputervision2016]. This map represents an capability represents an advance over Although free and providing higher radiometric quality and depth, a moderate-resolution sensor such as Sentinel-2 may be too infrequent to successfully penetrate clouds in an area such as southern Ghana (meaning that dry season imagery is likely to be all that is available), while lacking the spatial resolution necessary for labelling and classifying smaller fields.

A possible reason for the discrepancy between earlier maps and ours is that those earlier maps include more areas of active or abandoned cropland in their estimates. 


using a mapping system that is designed to work with high spatio-temporal resolution small-sat data.  
