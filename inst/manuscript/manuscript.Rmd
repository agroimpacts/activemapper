...
## System components
### Image compositing
The image processing components of our system was designed to work with PlanetScope Analytic surface reflectance data [@PlanetTeamPlanetApplicationProgram2017]. PlanetScope provides three visual (red, green, blue) and a near-infrared band at 3-4 m resolution at nominal daily frequency. Although these images are already pre-preprocessed and corrected for atmospheric effects, there are residual errors from inter-sensor differences and the radiometric normalization process [@HouborgDailyRetrievalNDVI2018], variation in the orientation of scene footprints, as well as a high frequency of cloud cover over the study region [@WilsonRemotelySensedHighResolution2016]. To correct for these factors, we developed a procedure for creating temporal composites representing the primary growing and non-growing seasons within a single year.  

PlanetScope imagery is accessed via the Planet API, and an initial order is placed for all imagery falling within the mapping geography and the date ranges for the two compositing periods. The imagery is collected and transferred directly to the system's cloud storage (Amazon Web Services [AWS] S3). 

Individual images are then transformed into "analysis ready data" (ARD) [@DwyerAnalysisReadyData2018], by cropping each downloaded scene into a 0.05 degree tile, regardless of cloud cover. Tiles are organized within a larger 1 degree resolution grid that covers the entire continent, which defines the minimum mapping area of interest (AOI; Figure \ref{fig:aois}). 
...
\mathrm{W2_t} =\begin{cases}
    \frac{1}{\mathrm{NIR_t}^4}, & \text{if $\mathrm{NIR_t}$ < median\{$\mathrm{NIR_1}$, $\mathrm{NIR_2}$, ..., $\mathrm{NIR_i}$\}}.\\
    1, & \text{otherwise}.
  \end{cases}
\end{equation}
...
#### Classification
Once the features from the training sites are extracted into `RasterFrames`, these are combined with their corresponding labels and passed to the machine learning classifier, a `SparkMLlib` implementation of Random Forests [@BreimanRandomForests2001]. For this study, the model was trained with a balanced sample and a tree depth of 15 and total tree number of 80, which initial testing showed to provide a reasonable balance between computational time and performance.

After fitting, the model is applied to the features of the model validation set, and a set of performance metrics are calculated, including binary accuracy, the F1 score (the geometric mean of user's and producer's accuracy), and the area under the curve of the Receiver Operating Characteristic [@Pontiustotaloperatingcharacteristic2014]. 
...
After scoring with the uncertainty criterion, the top *N* most uncertain grid cells are selected and sent back to the labelling platform, which are then digitized by the labellers. The resulting consensus labels from the actively selected sample are added to the initial randomly selected sampe, and a new cluster is launched. The model is retrained, assesses uncertainty across the remaining unlabelled sites, and selects the next most uncertain sites for labelling. This loop repeats until model performance gains against the validation set show diminishing returns.  

### Segmentation
After the final iteration, the segmentation algorithm is invoked, which entails several steps. In the first step, the meanshift algorithm [@YizongChengMeanshiftmode1995a] is applied to the original bands of the dry season composite. A Sobel filter is then tapplied to the green, red, and near-infrared mean-shifted bands and the probability map, and an combined edge image is computed using the sum of these four edge images for the dry season only. A compact watershed algorithm [@neubertCompactWatershedPreemptive2014] is then run on the weighted edge image, with a high level of segmentation specified. In this case, we specified **6400** segments per tile. 
...
To assess the accuracy of the final segmented boundaries, we used a two-step approach. First, we assessed the overall thematic accuracy of the resulting classification against our map reference data. Second, to assess the quality of the segmentation, we compared the mean area and relative frequencies of the segmented polygons within different size classes against the same metrics derived from the digitized fields of the most accurate worker to create the given map. We selected this relatively simple procedure, as opposed to more complex measures of object accuracy [@yeReviewAccuracyAssessment2018], because, on the one hand, both the automated segmentation algorithm and labellers are cueing in on the same features--abrupt, physically detectable breaks within the imagery. On the other hand, no matter how well the intepreted/segmented boundaries align with the boundaries of fields in the imagery, it is logistically difficult and potentially impossible to evaluate performance against the boundaries that farmers use to delineate their fields, as the conception of what constitutes a "field" could vary between farmers, while their field edges may be invisible or span multiple units that appear to a segmentation to have distinct boundaries. 

## Applying the system to map Ghana
To create a map of Ghana, we divided the country into 16 distinct AOIs, which were developed by grouping together each one degree cell fully contained within Ghana with the tiles belonging to any adjacent degree cell that overlapped neighboring countries (Figure \ref{fig:aois}A). The exception was AOI 16, which consisted of the four degree cells along Ghana's southern coast. The resulting AOIs ranged from 12,160 to 23,535 km$^2$ in extent (average = 15,457 km$^2$), A separate active learning and segmentation process was run for each of these AOIs. 

To collect the initial randomized samples for model training, we grouped the AOIs into three blocks: a northern block comprising the 6 northernmost AOIs, a central to southeastern blocks comprising the 3 middle (AOIs 7-9) and 2 southeastern AOIs (12 and 15), and a southwestern block made up of the forest zone AOIs (10, 11, 13, 14, 16). Within each block, we randomly selected and labelled 500 grid cells, which provided relatively large initial training samples for these agro-ecologically similar regions, while helping to minimize the overall amount of labelling effort. In addition to these samples, we randomly selected and labelled 100 grid cells within each AOI to provide a validation sample. 

After collecting the initial training and validation samples, we trained a starter model for each block and applied it to each of the block's AOI. For each iteration, 100 samples were actively selected within each AOI, and added to the training pool.  

To evaluate the performance of the system, we undertook several different analyses. 
...
We evaluated the overall quality of the resulting seasonal image composites by assessing a random selection of `r tilen` tiles. We graded both seasonal composites for each tile using a four category quality score, which evaluated the degree of 1) residual cloud and 2) cloud shadow, 3) the number of visible scene boundary artifacts, and 4) the proportion of the image that had its  resolution was degraded below the typical 3-4 m PlanetScope resolution (e.g. because of between-date image mis-registrations). Each category was qualitatively ranked from 0-3, with 0 being the lowest quality, and 3 the highest (see SI for complete protocol), making the highest possible score 12. We rescaled scores to fall between 0 and 1. 
...
To quantify the potential impact of label error on our results, we evaluated the performance differences between models trained with three different sets of labels: 1) those from the lowest scoring labeller to map each training site, 2) those from the highest scoring labeller, and 3) the consensus labels. This assessment follows recommended Tier 1 (i.e. best practice) standards to account for training data errors [@ElmesAccountingtrainingdata2020].  
...
### Accuracy assessment
The model performance assessments described above (2.3.2-3) were not fully independent because they used the same validation sites over multiple iterations [@ElmesAccountingtrainingdata2020]. To independently assess the accuracy of our final map products, we followed recommended guidelines [@StehmanKeyissuesrigorous2019] to create a separate map reference sample. To develop this sample, we first defined cropland and non-cropland strata for Ghana by intersecting the centroids of the 0.005 degree grid (Figure \ref{fig:aois}C) with the segmented field boundaries, and then randomly selected samples from the points representing each stratum. The selected sample was loaded into labelling platform, were it was presented as a ~0.1 ha square polygon (representing the minimum mapping unit). An expert supervisor who was not involved in labelling evaluated the PlanetScope composites at each site to determine its class membership. If the 0.1 ha target was not completely contained within a single class, the supervisor drew a square polygon of ~0.1 ha and shifted it the minimum distance necessary to make it contained entirety within the class that the initial target had the greatest overlap. Four classes were used for the map reference sample: cropland; non-cropland; unsure but likely cropland; unsure but likely non-cropland. The latter two classes were used to provide insight into the degree of uncertainty in the map reference sample. For efficiency, two separate supervisors evaluated separate portions of the reference sample, but both assessed a small subset. We calculated their level of agreement on this subset to provide an additional assessment of uncertainty in the map reference sample [@StehmanKeyissuesrigorous2019]. The SI contains further details on the creation and analysis of the map reference sample. 
...
Using the final mapped results, we calculated the estimated area of arable cropland in Ghana, as well as the average size of fields and number of fields in the different AOIs. We used the map reference sample to calculate adjusted area estimates and confidence intervals for each map class, and used the biases between labellers' polygons and segmented boundaries at validation labels to calculate adjusted estimates of mean field sizes and the total number of fields. 
...
A second measure of the impact of training data error can be obtained by calculating the between the average training label risk per AOI versus various model performance metrics (Table S3). Accuracy had a strong negative association (Spearman's Rank Correlation =  `r risk_corr[, "Accuracy"]` and AUC had a moderate negative association (r = `r risk_corr[, "AUC"]`), while F1 had a weaker positive association (r = `r risk_corr[, "F1"]`). This sign of the latter is counter-intuitive, but relates to the fact that precision was higher in areas of increasing label risk (`r risk_corr[, "Precision"]`), as did false positive error rates (`r risk_corr[, "FPR"]`). In contrast, recall had little correlation with label risk (`r risk_corr[, "Recall"]`), thus the higher precision drove F1 higher, even in areas with more false positives.  **[work on this]**
...
To assess the overall accuracy of the final maps, we used our map reference sample of `r sum(ref_ct)` sites (`r sum(crp_ct$n)` cropland; `r sum(noncrp_ct$n)` non-cropland) to evaluate the accuracy of 1) pixel-wise classifications produced by the RandomForest model, and 2) the segmented field boundary maps. We first evaluated the uncertainty in the map reference classes using two approaches, the first being an assessment of the overall agreement between map reference labels (at `r sum(usertab)` sites), and the second was the level of confidence in assigned labels (see SI for details). The first measure showed that the two individual labellers agreed `r agreement`% of sites, while the second showed that `r round(sum(crp_ct$n[2], noncrp_ct$n[2]) / sum(ref_ct) * 100, 1)` of sites labelled were of lower confidence.  

Based on this reference sample, we found that the overall accuracy of the pixel-wise classifications was 88% (Table \ref{tab:mapaccuracy}). This overall level of accuracy varied within four smaller zones (Figure S9A), ranging from 83.3% in Zone 1 (comprised of AOIs 1-3) to 93.6% in Zone 3 (AOIs 10, 11, 13, 15, and 16). Examining the accuracies of the individual classes, the Producer's accuracy of the cropland class was 61.7% across Ghana, ranging from 45.6% in Zone 3 to 67.9% in Zone 1. The User's accuracy for cropland was 67.3% overall, ranging from 59.8% in Zone 4 to 71.2% in Zone 1. Both measures of accuracy were substantially higher for the non-cropland class across all zones, typically exceeding 90%, with the lowest accuracies for this class found in Zone 1 (Producer's = 89.3% and User's = 87.7%, respectively). 

The overall accuracies obtained from the segmented maps were generally 1-2 percentage points lower than those of the per-pixel maps, while User's accuracies tended to be 8-10 percentage points less (Table \ref{tab:mapaccuracy}). In contrast, Producer's accuracies were 15-20 points higher than in the per-pixel map, indicating that the segmentation step helped reduce omission error, but substantially increased false positive errors.  
...
\begin{table}
\caption{Map accuracies and adjusted area estimates for the ~3 m pixel-wise classifications (based on RandomForest predictions; top 5 rows) and the segmented map (bottom 5 rows). Results are provided for 4 zones (Zone 1 = AOIs 1-3; Zone 2 = AOIs 4-9; Zone 3 = AOIs 10, 11, 13, 14, 16; Zone 4 = AOIs 12, 15) plus the entire country. The error matrix (with reference values in columns) provides the areal percentage for each cell, and the producer's (P), user's (U), and overall (O) map accuracies and their margins of error (in parenthesis) are provided, as well as the sample-adjusted area estimates (in km$^{2}$) and margins of error. }
...
## Ghana's croplands
Two separate maps of cropland were produced for each AOI, the per-pixel classified from the cropland probabilities, and the vectorized map of field boundaries (Figure \ref{fig:mainmap}). The former map provides the more accurate assessment of the distribution of croplands in Ghana, which are most concentrated in the Southeastern corner (AOI 15), the central-western region (AOI 7, the northeastern and northwestern corners of AOIs 10 and 11, and the south of AOI 8), the northeastern quadrant stretching from AOI 9 through 5 and 6 up to 2 and 3. The northern third of AOI 1 also has noticeable densities of cropland. Several noteable areas of low cropland density are also apparent, indicating the presence of large protected areas, such as Mole National Park in the the southeastern corner of AOI 1 and Digya National Park in the northwestern corner of AOI 12. In contrast, the relative absence of cropland in AOIs 13, 14, and 16 do not reflect the scarcity of agriculture, but rather the predominance of tree-based agriculture in the forested agroecoregions of Ghana, which was not mapped. 

```{r mainmap, echo = FALSE, out.width="90%", fig.cap="The distribution of arable croplands in Ghana. The main map shows the percentage of croplands in each 0.005 degree grid cell, derived from the predicted cropland probabilities. The insets  on the margins illustrate predicted probabilities (top map in each couplet) at original resolution (0.000025 degrees) and segmented field boundaries overlaid on the dry season PlanetScope composite, for four separate tiles. Each tile's position is shown on the main map, and is color-coded to the boundary lines around its corresponding inset.", fig.align='center', message=FALSE}
...
Both the per-pixel and vectorized maps, when combined with the map reference sample, enable separate estimates of the total extent of arable croplands in Ghana. The cropland extent estimated from the vectorized map is `r fmt(segas[2])` km$^2$ (with a margin of error of `r fmt(segma[1])` km$^2$), or `r segcipct[1]` (`r paste(segcipct[2:3], collapse = "-")`%) of the mapped area. The estimate based on the per pixel map is `r fmt(rfas[2])` km$^2$ (margin of error = `r fmt(rfma[1])` km$^2$), or `r rfcipct[1]` (`r paste(rfcipct[2:3], collapse = "-")`%) of area. 


The vectorized map provides additional information on how the characteristics of croplands can vary geographically, ranging from narrow, strip-like fields in parts of AOI 15 (Figure \ref{fig:mainmap}'s lower right inset) to more densely packed, less distinctly shaped fields in area such as AOI 5 (upper right inset in Figure \ref{fig:mainmap}). To explore how field characteristics varied geographically, we mapped the average field size and total number of fields within each 0.05 degree tile (Figure S12). These patterns generally correspond to those seen in the cropland density map (Figure \ref{fig:mainmap}), with larger sizes and field counts generally aligning in the areas of higher field densities, although the biases inherent in both measures (Figures S10-11) complicate the interpretation of those variations. However, we can use the estimated to develop adjusted estimates of field sizes and counts for each AOI, and for Ghana as a whole (Table \ref{tab:sizentab}). The bias-adjusted estimates show that the typical field size in Ghana is 1.73 ha, ranging from 0.96 in AOI 4 to 2.82 ha in AOI 4, with fields in the forest zone AOIs (10, 11, 13, 14, 16) generally smaller than those in the northern half of the country (Table \ref{tab:sizentab}). The total number of fields was estimated to be 1,662,281 overall, or 205 fields per tile on average, ranging from 108/tile in AOI 4 to 399/tile in AOI 6. 
...
These results demonstrate our system's ability to create high resolution maps of the distribution and characteristics of cropland across a diverse and hard-to-map agricultural systems, at country-scales and at sub-annual to annual time steps. Our maps provide an updated and more granular view of the distribution and extent of arable croplands in Ghana, complementing existing national to regional land cover maps derived from moderate resolution sensors [@ESAESACCILAND; @hackmanNewLandcoverMaps2017; @XiongNominal30mCropland2017]. These earlier maps estimated that cropland covered `r round(4.62 / 23.86 * 100, 1)` [@XiongNominal30mCropland2017] to 32% [@hackmanNewLandcoverMaps2017] of Ghana's areas in the 2015, in contrast to our 2018 maps, in which cropland covers 16.1-23.2% of the mapped area (Table \ref{tab:mapaccuracy}), with sample-corrected estimates of `r segcipct[1]`-`r rfcipct[1]`%. Our findings suggests that Ghana's arable cropland is slightly less than previous estimates, although the difference may also be attributable to the earlier maps using a more expansive definition of cropland than ours (active to recently active fields). 

In addition to the more detailed update of cropland extent, our maps also provide new information on the size and number of fields in Ghana (Figures \ref{fig:mainmap}, S12). Prior efforts to map such agricultural characteristics have often focused on estimating farm rather than field size using census datasets [@jayneAfricaChangingFarm2016; @LowderNumberSizeDistribution2016; @SambergSubnationaldistributionaverage2016; @VonBraunSmallscalefarmersliberalized2004]. Efforts to map fields in small-scale agricultural systems have either been field-based [@CarlettoFactartifactimpact2013; @CarlettoGuesstimatesGPStimatesLand2015] or remote sensing studies over relatively small areas [e.g. @forkuorIntegrationOpticalSynthetic2014] (**include Persello**). The most notable efforts to estimate field sizes were two global initiatives that used crowdsourced volunteers to categorize five broad field size classes based on their interpretations of imagery sampled from virtual globes [@lesivEstimatingGlobalDistribution2019; @FritzMappingglobalcropland2015]. The most recent of these [@lesivEstimatingGlobalDistribution2019] provided country-specific results for Ghana (n = 263) that return an average field size of `r round(lesiv_est, 2)` ha (calculated as the weighted mean of the class counts and the middle values of the four smallest size classes and the lower bound of the largest size class). This estimate exceeds our Ghana-wide average segment size (3.92 ha; Table \ref{tab:sizentab}), but is closer to the mean segment size (`r aoi_nofor` ha) across AOIs 1-9, 12, and 15, which is where most of their sample appears to have been collected. However, our bias-corrected estimates of 1.73 (Ghana-wide) and `r aoi_nofor_adj` (AOIs 1-9, 12, and 15) ha are much smaller. 

## Sources of error
Although the maps generated by our system provide valuable new information, they nevertheless contain substantial errors. The overall map accuracies are good (86.7-88%, Table \ref{tab:mapaccuracy}), which is at the boundary of what could be considered achievable map accuracy [@ElmesAccountingtrainingdata2020], given the level of confidence in the map reference sample (only 85% of reference labels were assigned with confidence). However, accuracies for the cropland class were between 62 (Producer's) and 67 (User's) percent in the per-pixel map across the country, indicating that both commission and omission errors were above 30% for this class. The field boundary has lower omission error (Producer's accuracy = 79%), but that comes at the expense of increased false positives (User's accuracy = 58.2%). These accuracies are near the middle to upper ranges of those reported for the cropland class in other large-area mapping studies [@hackmanNewLandcoverMaps2017; @LesivEvaluationESACCI2017; @XiongNominal30mCropland2017]. 

The patterns of cropland-class accuracies varied by zone, which largely align with agroecozones and thus somewhat reflective of the inherent challenge each poses for mapping. Producer's accuracies for both maps were highest in the two northern zones (1 and 2), which are primarily savanna agroecoregions (Figure S9), and lowest in zones 3 and 4, which are either forest or coastal savannas. User's accuracies followed a similar pattern, with the exception of Zone 3, which had the highest User's accuracy, albeit from a very small sample. Confining the samples to specific agroecozones provides further insight into error patterns (Table S4). The coastal savannas in the southeast had the highest Producer's accuracy but lowest User's accuracy for the per-pixel map, presumably due to the numerous areas of high density cropland and low woody cover in surrounding uncultivated areas, which promotes commission error. The northern savannas had the best balance between omission and commission error, and had the highest overall User's accuracy. The transition zone had a very low Producer's accuracy (21%), which likely reflects the fact that this region, which also has high cropland density, was divided between several AOIs for mapping. 

The relatively low accuracies for the cropland class can be attributed to several important factors. The first of these was the overall mapping extent combined with the resolution of the analysis. Given the goal of country-scale mapping at high resolution, we found that the computational cost required us to use a relatively small and simple set of features and less than the recommended tree number and depth [@MaxwellImplementationmachinelearningclassification2018] in our Random Forest implementation. Previous work shows that RandomForest achieves much better performance on small-scale croplands when trained on a large number of randomly generated features [@Debatsgeneralizedcomputervision2016], which is a commonality with deep learning. However, applying such a large feature set within the extent of our AOIs was intractable, as the computing time would have been several-fold larger than the typical 10-14 hours required from training through prediction on 800 CPUs, for a single iteration. This left the model less able to discern cropland from surrounding savanna vegetation with bare patches or sparse grass cover, which was common in most AOIs. A larger feature set with more variable textures filters could have helped distinguish such areas from crop fields, helping to particularly reduce commission error. 

Another major limiting factor was the inherent difficulty of the labelling task. Our system was designed to minimize the error inherent in labelling, but determining croplands from non-croplands over these system is difficult and requires the evaluation of multiple image sources and considerable judgement by each labeller. Interpretation is particularly hard over areas where the background savanna vegetation and croplands have similar reflectance during the dry season, which is a particular problem in AOIs 2 and 3. Such changes in scene circumstances require labellers to adopt different heuristics, for example by delineating the field boundary on the growing season rather than the dry season image. Smaller field sizes also complicated the labelling task, becoming increasingly indistinct in the ~4 m PlanetScope composites. The difficulty of labelling is reflected in the magnitude of the Bayesian Risk metrics (Figure S5), and by labeller accuracies that averaged 71% (Figure S4B). The design of our system allowed us to measure how much label quality impacts model performance. Although prior work [@mellorExploringIssuesTraining2015; @rodriguez-galianoAssessmentEffectivenessRandom2012] has reported that RandomForest is particularly robust to training data error, we found its impact to be substantial (Figure \ref{fig:trainingimpact}), suggesting that the greatest single investment towards improved accuracy lies in improving training data quality. 

Image quality was another issue, although primarily in the forest zone (Zone 4) AOIs, where frequent cloud cover reduced the number of available images in all seasons, resulting in composites with more brightness artifacts and blur (Figure \ref{fig:imqual}). This impacted labellers' abilities to discern arable fields, and doubtless affected model predictions. There is little to be done to mitigate these errors, short of confining imagery to the less cloudy dry season, which may further undermine model performance--prior work shows that imagery from multiple seasons is important for improving classifier performance [@Debatsgeneralizedcomputervision2016; @Defournyrealtimeagriculturemonitoring2019]. Composite quality could be improved by integrating imagery from the same seasons over multiple years, but this would undermine the goal of developing annual maps, while the dynamism of the croplands would confound the ability to delineate field boundaries. 

The final major source of error arose from the segmentation process. The vectorized maps had high commission errors that are attributable to the RandomForest predictions. Uncertainty in that model resulted in substantial numbers of pixels just above the 0.5 classification threshold, meaning a number of segments over highly uncertain areas were retained as fields. A more accurate classifier would reduce such errors. Another possible solution could be to apply a locally varying classification threshold to distinguish croplands from non-cropland [e.g. @waldnerDeepLearningEdge2020]. Over-merging was another source of error in the segmentation algorithm. This led in some areas to over-estimation of field sizes and often irregular shapes, particularly in high density croplands (e.g. in AOIs 2 and 8; Figure \ref{fig:mainmap}) where adjacent fields have boundaries that are indistinct in the PlanetScope imagery. Minimizing or preventing merging would help in such cases, although it would result in  over-segmentation and thus would likely underestimate field sizes. However, over-segmentation can help to identify important within-field distinctions (e.g. different soil types), which can be useful for other applications, such as filtering pixels for crop type mapping or yield estimation [e.g. @EstesComparingmechanisticempirical2013; @EstesProjectedclimateimpacts2013]  

## Key system features
Despite the sources of errors mentioned above, the design of the system proved effective in mitigating error, resulting in a higher overall accuracy than would have otherwise been possible. The label accuracy assessment and consensus labelling methods appeared to be the most effective tools for error mitigation. Label accuracy assessment allowed us to quantify the impact of trainnig data error on model performance, in keeping with recommended best practices [@ElmesAccountingtrainingdata2020]. Label quality proved to have a large impact on model performance (Figure \ref{fig:trainingimpact}), and consensus labeling was effective in reducing individual digitization errors and between-labeller differences in interpretation, improving label, and thus map, quality. Worker-specific label accuracy measures also helped to improve the overall quality of the consensus labels, by enabling higher weight to be placed on labels that were more likely to be accurate during the merging process, rather than giving equal weight to potentially less accurate labels. The ability to select the most accurate individual labels for a site also allowed us to develop independent estimates of field size to which measures of confidence can be attached (Figure S4B), which we could in turn use to bias-correct map-based size class estimates (Table \ref{tab:sizentab}). The ability to select the most accurate label will also help when different training strategies are needed, such as when training a model to perform instance segmentation [@carvalhoInstanceSegmentationLarge2021], in which case the field identities have to be preserved.  
...
```
The active learning approach helped to improve overall model performance relative to a randomized approach to training site selection. Although we only compared active to random site selection within three AOIs, the average gains across all three AOIs and three performance metrics demonstrated the effectiveness of the active approach, most notably by the gains in the F1 score (Figure S7), a combined metric of commission and omission error. The gains in accuracy and AUC were smaller. For accuracy, this was due to the larger drop in the active learning metric after iteration 1 in AOI 15, which proved one of the hardest AOIs to map. The plateau in AUC gains at 0.5% relative to random training reflects the fact that each iteration tended to reduce both the false and true positive rates simultaneously. Although the false positive rate overall declined (an average of `r prdelta$rate[1]`% between Iterations 0 and 3), the reductions in true positive rate (averaging `r prdelta$rate[2]`%) offset some of the improved predictive power. 

The gain due to active learning were substantially less than those reported by Debats et al (2017), who found that active site selection improved classifier performance by `r round((0.45-0.35) / 0.35 * 100)` over randomized site selection after one iteration, and after 45 iterations by `r round((0.69-0.64) / 0.64 * 100)`%. These larger gains likely reflect the fact that this evaluation began from the smallest possible training sample--one image chip--and tested the effect of adding a single additional chip each iteration. In contrast, we started with a large initial random sample (500) and added 100 samples per iteration, thus our models received significant training at the outset, making the marginal gain due to each new sample smaller. This suggests that we could have benefitted by reducing the size of the initial random training sample while increasing the size of actively selected samples, not only in terms of larger accuracy gains, but also in terms of reducing--by as much as 62%, per Debats et al (2017)--the overall size of the training sample needed to achieve a given level of performance. 

The detail, large extent, and temporal precision of the maps we created by virtue of our system's intentional design around PlanetScope imagery, which currently provides the only wall-to-wall, sub-5 meter imaging capability [@McCabefutureEarthobservation2017] with daily frequency. Daily frequency is important for creating multiple seasonal composites during a single agricultural year in cloudy regions. Although unquantified, our compositing technique may be more effective at providing complete image coverage during a season than the "best-on-top" approach\footnote{https://assets.planet.com/products/basemap/planet-basemaps-product-specifications.pdf} used by Planet to develop their analytic basemap, which anecdotally appears to contains substantial residual cloud cover during the growing season in southern Ghana. Although free and providing higher radiometric quality and depth, and of sufficiently high resolution (10 m) to accurately classify small-scale agricultural systems [e.g. @Defournyrealtimeagriculturemonitoring2019; @kernerRapidResponseCrop2020], Sentinel-2's 5-day revist may be too infrequent to provide enough clear pixels to create a cloud-free growing season composite in the southern half of Ghana. Sentinel-1 is not affected by the same problem, but labelling fields in a more coarsely resolved radar images is difficult.  

## Lingering questions
Several potential issues not answered by our assessment merit further exploration. These include the correspondence between image- and ground-collected labels. We believe that such comparisons will in most circumstances reveal unresolvable discrepancies. The highly dynamic nature of the systems in question means that even narrow temporal discrepancies between ground and image-collected labels can produce substantial differences. These discrepancies could be further exacerbated by differences in an understanding of what constitutes a field, which might vary on the ground depending on the farmer or the data collecter's collectors judgement. On top of this, field-collected boundaries are typically highly geographically constrained and limited in size, meaning that any comparison might suggest that the relationship is more noisy than it actually is. These factors suggest that ground versus image label differences would not necessarily indicate how far image-based labellers were from the "truth". Nevertheless, a comparison against ground data can help to assess how accurately image-collected labels capture the typical size of fields, and thus merits further investigation for this reason.  

The temporal discrepancies mentioned above [and discussed in @ElmesAccountingtrainingdata2020] are another reason why we chose not to  label on basemap imagery (in addition to restrictive usage terms), which is typically several years old [@LesivCharacterizingspatialtemporal2018]. However, we did not assess whether the higher label accuracy one might achieve by digitizing on <1-2 m basemap would offset model errors due temporal mismatches between labels and imagery. 

Another potential issue is the degree to which our assessment of the impact of label error on model performance (Figure \ref{fig:trainingimpact}) was influenced by the validation labels we used, which were generated using the consensus method. This could have confounded the assessment, particularly when comparing models trained with the most accurate individual label and those trained with consensus labels. However, the visual assessment of probability images generated from each labelling strategy show that these two strategies produce nearly identical maps, and that the model trained with low quality labels generated a markedly less certain map (Figure S6). 

## Next steps
The maps presented here represent a version 1 product that is freely available to use **(links)**, along with its underlying code. These data were developed according to the recommended best practices for training and assessing error in machine learning models [@ElmesAccountingtrainingdata2020]. In their current form, the maps may be useful for a variety of research applications. For example, analyzing the  distributions of values in the probability maps may provide additional insight into the relative extents of active versus fallow croplands [@Tongforgottenlanduse2020]. However, use of these data, particularly for decision-making processes (e.g. cropped area estimates), should be careful to account for the reported errors [@OlofssonGoodpracticesestimating2014; @StehmanKeyissuesrigorous2019].

To facilitate the next step, generating more accurate version 2 maps, several improvements will be made. The first is to replace Random Forest with a more advanced convolutional neural network (CNN), which can generate and learn from a large number of features representing a variety of spatial scales [@maDeepLearningRemote2019].  Recent work suggests that a common architecture such as U-Net, when trained to distinguish field edges from interiors and combined with a post-hoc segmentation routine, provide an effective means for delineating field boundaries [@waldnerDeepLearningEdge2020]. Our system can readily incorporate such a model. The active learning framework is already analagous to transfer learning, in which a trained deep learning model is applied to  a different region and updated with local training samples [@panSurveyTransferLearning2010; @russwurmMetalearningFewshotLand2020]. The existing tooling can thus be adjusted to guide training site selection in sub-regions of Ghana for a CNN trained from scratch from a larger initial random draw. Instead of the current AOIs, a more effective strategy may be to update the model within agroecozones (Figure S9B) The labelling methods already provide for a case in which edge and interior classes need to be labelled, as the most accurate individual training samples can be identified and used, rather than the consensus strategy. The segmentation algorithm will also be updated to work with the resulting score maps.  


## Conclusion
This work demonstrates the proof of concept for developing high resolution, annual maps of small-scale croplands over national to regional scales, using a flexible and framework that can be readily updated to improve their accuracy as technologies improve. Such maps will be important for developing an understanding of the rapid agricultural change that is currently unfolding throughout much of the continent.  


# Acknowledgements
The primary support for this work was provided by Omidyar Network's Property Rights Initiative, now PlaceFund. Support for computing was provided by the AWS Cloud Credits for Research program and the Amazon Sustainability Data Initiative. Additional support was provided by NASA (80NSSC18K0158), and the National Science Foundation (SES-1801251; SES-1832393). We thank Joe Okyere and his colleagues at Meridia for assistance in data collection. 
