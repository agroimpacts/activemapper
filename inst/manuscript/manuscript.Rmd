...
- affilnum: 1
  affil: Graduate School of Geography, Clark University, 950 Main Street, Worcester, MA, USA
...
```
The active learning approach helped to improve overall model performance relative to randomized training site selection, confirming previous findings reported by @DebatsIntegratingactivelearning2017. Although the performance gains relative to randomized model training that we observed were substantially less than in the previous study, which reported `r round((0.45-0.35) / 0.35 * 100)`% higher model performance after one iteration, and `r round((0.69-0.64) / 0.64 * 100)`% higher after 45 iterations [@DebatsIntegratingactivelearning2017], those comparisons were made from a low initial base, beginning with a single training site with one more site added each iteration. In contrast, our large initial random sample (500) meant that our models were substantially trained before they were exposed to actively selected labels, thereby diluting their impact on performance. Nevertheless, the higher average performance of the active approach across three performance metrics demonstrated its effectiveness. Most notable were the larger gains in the F1 score (Figure S7), a balanced performance metric. The gains in accuracy and AUC were smaller. For accuracy, the reduced advantage was primarily due to active learning being outperformed by randomized training after the first iteration in AOI 15, which proved one of the hardest AOIs to both map and label. Active learning likely resulted in the selection of sites that were harder to label than randomly selected ones, leading to more label error, and thus initially lower model accuracy. However, this deficit was overcome by the 5th iteration. The plateau in AUC gains at 0.5% better than randomized training reflects the findings that active learning reduced both the false and true positive rates, the two inputs to AUC. Although the false positive rate declined by `r prdelta$rate[1]`% between Iterations 0 and 3, the `r prdelta$rate[2]`% reduction in the true positive rate offset some of these gains, and the effect of this tradeoff may have been more pronounced in the threshold independent AUC. 

The detail, temporal precision, and large extent of these maps was enabled by the system's ability to process PlanetScope data, which is currently the only source of sub-5 meter imagery with daily coverage [@McCabefutureEarthobservation2017]. Daily revisits are necessary for creating seasonal composites within a single agricultural year over cloudy areas. The compositing technique we developed to work with PlanetScope was able to provide complete image coverage over the entire country for both seasons. Although Sentinel-2 is free, has better radiometric quality, and has sufficiently high resolution (10 m) to accurately classify small-scale agricultural systems [e.g. @Defournyrealtimeagriculturemonitoring2019; @kernerRapidResponseCrop2020], its 5-day interval is too infrequent to generate sufficiently cloud-free composites during the growing season over southern Ghana. Sentinel-1 is not affected by the same problem, but labelling fields in more coarsely resolved radar images is difficult. 

## Lingering questions
Several potential issues not answered by our assessment merit further exploration. One of these is the degree of correspondence between image- and ground-collected labels. However, such comparisons will likely reveal unresolvable discrepancies in most circumstances. The highly dynamic nature of the systems in question means that even narrow temporal discrepancies between ground and image-collected labels may show substantial differences. These discrepancies could be further exacerbated by differences in an understanding of what constitutes a field, which might vary on the ground depending on the farmer or the data collecter's collectors judgement. On top of this, field-collected boundaries are typically highly geographically constrained and limited in size, meaning that any comparison might suggest that the relationship is more noisy than it actually is. These factors suggest that ground versus image label differences would not necessarily indicate how far image-based labellers were from the "truth". Nevertheless, a comparison against ground data can help to assess how accurately image-collected labels capture the typical size of fields, and thus merits further investigation for this reason.  
