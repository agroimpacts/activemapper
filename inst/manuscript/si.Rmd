...
#### Worker's assignment evaluation
We assigned a score for each workers' assign; the score history determines the weights of each workers for creating a consensus label for each 0.005 degree resolution grid. The score for a particular assignment *i* is defined as a weighted sum of five accuracy metrics:
...
*E* is the average edge accuracy for all pairs of matched workers and refernece polygons; the edge accuracy for a single pair is defined as the length of 'correctly mapped edges' (*L$_c$*, the partial boundary of a workers' polygon that are within a three-planet-pixel buffer region of the matched reference polygon boundary) over the total boundary length of its matched reference polygon (*L$_t$*).
