...
The distribution of data collection effort and scores received at training reference sites by labelling team (Figure \ref{fig:trainqual}). These results include those from a repeated labelling of the initial training in Cluster 2.  This was done because we discovered a small spatial offset in the original image composites, which we corrected by reprocessing the images. We replaced the image overlays in the instance for Cluster 2 \ref{fig:trainval}A), and the labelling team mapped these sites a second time during the production run in late 2019. The reprocessed labels were used to initially train the model for AOIs 9, 12, and 15.
