...
To examine average field sizes, the average area of the vectorized field boundaries intersecting each 0.005 degree cell in the mapping grid was calculated and mapped back onto the grid (Figure \ref{fig:fieldsize}A). Given the overestimation bias inherent in the vectorized boundaries relative to the hand-digitized validation labels, we also used a regression kriging approach [@henglRegressionkrigingEquationsCase2007] to these average field sizes for this bias. To do this, we used the estimated mean field sizes extracted from the map in Figure S12A to predict the average polygon size of the most accurate mapper's labels for each of the 1600 validation sites, using an ordinary least squares regression to model the relationship, after first log-transforming both variables. We then applied ordinary kriging (using an exponential model for the variogram) to the regression residuals, which we added to the surface of the predicted field sizes to create the adjusted surface, and then converted this back to units of hectares (Figure \ref{fig:fieldsize}B).  

```{r fieldsizes, echo = FALSE, out.width="100%", fig.cap="Average fields sizes in Ghana at 0.005 degree resolution, as calculated from A) the vectorized field boundaries, and after B) adjustment against hand-drawn validation labels using a regression-kriging approach.", fig.align='center', message=FALSE, results='hold'}
knitr::include_graphics('figures/si_fsizes.png')
```
