...
- [`activemapper`](https://github.com/agroimpacts/activemapper): Repository\footnote{github.com/agroimpacts/activemapper} containing code and derived data used to write this paper. 
- [`imager`](https:github.com/agroimpacts/imager): Repository\footnote{github.com/agroimpacts/imager} containing code for the image processing software.
- [`labeller`](https:github.com/agroimpacts/labeller):  Repository\footnote{github.com/agroimpacts/labeller} containing code for the labelling platform. 
- [`learner`](https:github.com/agroimpacts/learner): Repository\footnote{github.com/agroimpacts/learner} containing code for the active learning component.
- [`segmenter`](https:github.com/agroimpacts/segmenter): Repository\footnote{github.com/agroimpacts/segmenter} containing code for the segmentation algorithm.
- [`mappingafrica.io`](https://mappingafrica.io): Project website\footnote{https://mappingafrica.io} linking to web map displaying field boundary maps. The web map is currently undergoing redevelopment, and will be hosted on a new URL and links download maps. In the interim, map data are available on request.  
...
### Segmentation
An overview of the two inputs to and outputs from the segmentation algorithm are shown in Figure \ref{fig:segfig}. 

```{r segfig, echo = FALSE, fig.align='center', out.width="100%", message=FALSE, warning=FALSE, fig.cap="An overview of the inputs and key outputs from the five-step segmentation algorithm."}
knitr::include_graphics('figures/si_segmentation_workflow.png')
```
