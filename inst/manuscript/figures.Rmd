...
rf_tbl <- tibble(x = 1, y = 10, dat = list(rf_tabs)) 
...
colnames(seg_tabs) <- colnames(rfreports$North)
seg_tbl <- tibble(x = 1, y = 10, dat = list(seg_tabs)) 

# Assemble plots with maps
# Table theme
tt <- ttheme_gtminimal(
  base_size = 4.5, 
  padding = unit(c(0.2, 0.2), "char"),
  core = list(fg_params = list(hjust = 1, x = 1))
)

m <- ggplot(aoi_zones) + geom_sf(lwd = 0.1) + 
  geom_sf_text(data = st_centroid(aoi_zones), aes(label = zone), size = 2) + 
  theme_void() + theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))

t1 <- ggplot() + 
  geom_table(data = rf_tbl, aes(x = x, y = y, label = dat), table.theme = tt) + 
  ggtitle("Per-pixel classification") + 
  theme_void() + 
  theme(plot.title = element_text(hjust = 0.5, vjust = 1, size = 6))
t2 <- ggplot() + 
  geom_table(data = seg_tbl, aes(x = x, y = y, label = dat), table.theme = tt) + 
  ggtitle("Segmentation") +
  theme_void() + 
  theme(plot.margin = unit(c(0,0,0,0), "cm"), 
        plot.title = element_text(hjust = 0.5, vjust = 1, size = 6))

# gp <- cowplot::plot_grid(t1, m, t2, nrow = 1, align = "hv", 
#                          # rel_widths = c(0.2, 0.4, 0.5, 0.4, 0.2),
#                          rel_widths = c(1, 1, 0.8),
#                          # rel_heights = c(0.5, 1.5, 0.5),
#                          scale = c(0.25, 0.95, 0.25),
#                          label_size = 5, 
#                          label_x = 0.25, 
#                          labels = c("Per-pixel classification", "",
#                                     "Segmentation"))
gp <- t1 + m + t2
ggsave(gp, filename = pth("figures/figure7.png"), width = 7.25, height = 2.35, 
       units = "in", dpi = 300)
```
