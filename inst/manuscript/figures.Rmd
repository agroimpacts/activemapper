...
# plot(is.na(nflds))
# 
# fsizes_n_05 %>% filter(is.na(nflds))
# fsizes_n_05 %>% filter(is.infinite(nflds))
# plot(fsizes_n_05 %>% filter(is.na(nflds)))
thm <- function(x) {
  theme_void() + 
...
p2 <- ggplot(africa) + 
  ...
  # geom_stars(data = nflds) +
  geom_sf(data = aois, size = 0.1, fill = "transparent", col = "grey") +
  scale_fill_viridis_d(name = "ha", na.value = "transparent", labels = leg) +
  # scale_fill_viridis_c(name = "N", na.value = "transparent") +
  coord_sf(xlim = xylims[c(1, 3)], ylim = xylims[c(2, 4)]) + thm()
p2adj <- ggplot(africa) + 
  geom_sf(fill = "gray40", color = "grey", size = 0.25) +  
...
ggsave(op, bg = "transparent", width = 6, height = 4, units = "in", dpi = 300, 
...
### Figures S10 and S11: Segmentation statistics (grid scale)
...
### Figure S12: Average fields sizes
...
Both original as derived from segmentations and adjusted using regression kriging. 
...
fsizes_adj <- read_stars(
  system.file("extdata/mean_field_size005_adj.tif", package = "activemapper")
)
...
p <- ggplot(africa) + 
  geom_sf(fill = "gray40", color = "grey", size = 0.25) +  
  geom_stars(data = fsizes) +
  geom_sf(data = aois, size = 0.1, fill = "transparent", col = "grey") +
  scale_fill_viridis_c(name = "ha", na.value = "transparent") +
  coord_sf(xlim = xylims[c(1, 3)], ylim = xylims[c(2, 4)]) +
  theme_void() + 
  theme(legend.position = c(0.7, 0.07), legend.direction = "horizontal", 
        legend.key.height = unit(0.2, "cm"), 
...
ggsave(op, bg = "transparent", width = 6, height = 4, units = "in", dpi = 300, 
       filename = pth("figures/si_fsizes.png"))
