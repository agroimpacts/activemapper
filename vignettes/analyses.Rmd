...
# mask master_grid and convert to points
...
set.seed(111)
fsample <- fpoints %>% filter(class == 1) %>% sample_n(size = 800)
set.seed(111)
nofsample <- fpoints %>% filter(class == 0) %>% sample_n(size = 683)

ref_sample <- bind_rows(fsample, nofsample) %>% 
  mutate(class = as.character(class)) %>% st_as_sf(coords = c("x", "y"))

ggplot(ref_sample) + geom_sf(aes(color = factor(class)), size = 0.5)
