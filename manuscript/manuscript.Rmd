...
Upon completion of the labels, *labeller* automatically launches an ephemeral Elastic Map Reduce cluster consisting of tens to hundreds of instances (e.g. m4.xlarge), depending on the size of the mapping geography. 
...
**[Fill in with intro]**. The average number of iterations per active learning process was **3**, as overall performance gains, as assessed against validation sets, saturated quickly (see **Figure X** in SI). 
...
## System evaluation/experiment results
### Label quality and map accuracy
We used the reference labels to evaluate the accuracy of maps generated with the least and most accurate workers' labels, and then compared the resulting scores to accuracy assessed using the consensus labels. On average, there was a substantial difference between maps trained with the least and most accurate workers' labels, with the former being on average **X percentage points (range Y-Y%)** more accurate (**Figure 6**). Maps generated with consensus labels were on average **X percentage points (range Y-Y%)** more accurate than those created with the most accurate workers' labels.  


```{r, echo = FALSE, out.height = "3in", out.width="5in", fig.cap="Difference in overall, user's, and producer's accuracy between maps made with consensus labels, the least accurate worker's, and most accurate worker's labels. [This a hypothetical placeholder figure].", fig.align='center', message=FALSE}
...
To further evaluate label quality, we assessed the degree of similarity between worker polygons with the field boundaries collected by our ground validation team. Overall, we found that on average a worker's field boundary interpreted from a PlanetScope image had only an **X%** agreement (as determined by area of intersection over union) with the corresponding field-mapped boundary, and the average field size mapped by a worker was on average **X% smaller/larger** than those mapped on the ground. **However, these discrepancies are not necessarily an indication of how far workers were from the "truth", but maybe instead be at least partially an indication of how different the perspectives as to what constitutes a field when looking at an image versus seeing it from the ground (which in many cases was based on the field team's judgement, rather than farmer guidance).** 
