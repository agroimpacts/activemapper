...
Once the full set of training sites has been completed, two random draws of pre-determined size (typically **X%** each of the overall number of labels) are made from the randomly selected consensus labels. The first set is designated as a model test set, and the final set is set aside as final map validation sample. The remaining **Y%** of the labels are used to train the classifier. 

### Classification pipeline
Upon completion of the labels, *mapper* automatically launches an ephemeral Elastic Map Reduce cluster consisting of tens to hundreds of memory-optimized instances (e.g. m4.xlarge), depending on the size of the mapping geography. 

#### Feature extraction
The first step in the pipeline is the extraction of additional features from the growing season/non-growing season PlanetScope imagery. Previous work showed that a large number of simple features that summarize the statistical properties of reflectance and vegetatiation indices in local neighborhoods are highly effective for classifying smallholder croplands [@Debatsgeneralizedcomputervision2016]. We followed this logic in this study, but opted for a smaller feature space to accomodate the storage and memory requirements of a mapping geography orders of magnitude larger than that initial work. Here, we used a set of X features, comprised of the eight original composite bands (four from each season), a 5X5 and YXY mean of each band, as well as the slope of each image calculated within the 5X5 and YXY windows (Table X).
...
After scoring with the uncertainty criterion, the top *N* most uncertain grid cells are selected and sent back to *mapper*, where workers then digitize new labels. These new consensus labels are then added to *learner*'s initial training pool upon completion, and a new machine learning instance is launched and completed, and uncertainty scores for the remaining unlabelled sites are calculaetd, and the most uncertain sites are sent again to the workers for labeling. This loop repeats until the mapping skill as assessed with the model test set reaches an asymptote.  
...
To assess the accuracy of the final segmented boundaries, we used a two-step approach. First, we assessed the overall thematic accuracy of the resulting classification against our reserved validation set within each AOI. Second, to assess the quality of the segmentation, we compared the mean area and relative frequencies of the segmented polygons within different size classes against the same metrics derived from the digitized fields of the most accurate worker to map the given AOI. [**Note: the following maybe better for discussion**] We selected this relatively simple procedure, as opposed to more complex measures of object accuracy (**cite**), because both an automated and  human-interpreted segmentation worker interpreting imagery cue in on--abrupt, physically detectable breaks within the imagery--are the same. On the other hand, no matter how well these boundaries align the boundaries of fields in the imagery, it is logistically difficult and potentially impossible to evaluate such an approach against the boundaries that farmers use to delineate their fields, as the conception of a "field" may vary between farmers (**cite**) and their field edges may be invisible or span multiple units that appear to a segmentation to have distinct boundaries (**see SI**). 
 
# Results
We developed a cropland map for **X different [replace with whole country]**, agro-ecologically regions of Ghana (X km$^2$ total) (or whole of Ghana), ranging from the primarily grain and vegetable crop producing regions in the drier northern savanna regions to tree crop dominated regions in the forested southwestern corner of the country, where cocoa and oil palm are among the dominant crops. For these latter regions, we did not attempt to classify tree crops, and instead mapping clearing that potentially contain field crops or recently felled or newly replanted tree crops. Our rationale for this decision was that the resolution of PlanetScope imagery is insufficient to distinguish such crops from the surrounding forest, and clear field boundaries are typically not visible in cocoa crops. 
...
We evaluated the overall quality of the resulting composites across the region using a random selection of 100 tiles, which we graded according to an ordinal measure of image accuracy that ranged from 1 (lowest quality: significant residual haze/cloud and image edge artifacts that rendered visual interpretation difficult) to 5 (highest quality, no detectable haze/cloud and no image edge effects; see SI for full interpretation of categories. The results showed that the majority of the sampled composites (**X%; Fig Xb**) were rated 4 or higher, with small proportions falling within the lowest two quality categories (**Y and Z%**). These lower quality images were confined primarily to the growing season period and the Southwest corner, where the substantially greater cloud cover resulting in a much lower density of available PlanetScope imagery for each time period (**Fig Xc**). Overall, the available image density and quality of composites were closely related (see SI for full details).  
...
We ran 16 instances to create cropland probability maps for the **X test regions/whole of Ghana**. Each instance covered a distinct mapping geography comprised of one or more adjacent AOIs (Figure 5), ranging in extent from 12,160 to 23,535 km$^2$ (average = 15,457 km$^2$), and represented a separate active learning process. The average number of iterations per active learning process was **X (range = X-X)**, and the average overall accuracy was **X, with a range of X-X** (Figure 5). Overall user's accuracy was slightly **lower/higher** at **X, (range X-X)**, while producer's accuracy averaged **X, (range X-X)**. Each accuracy measure was highest in **northern-northeastern 2/3** of the country, and lowest in **southerwestern 1/3**, where rainfall and cloud-cover are highest, and where tree crops dominate.  
...
## Labelling accuracy
The accuracy patterns generally align with the difficulty of the labelling task, that is, **the southwestern regions were hardest for workers to interpret, as shown by Figure 6.**
